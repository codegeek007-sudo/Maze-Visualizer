 ...
 * Therefore it can’t move diagonally to a free cell passing between two obstacles
...
public class maze55 {
    ...
    public static class MazePanel extends JPanel {
        ...
        private class Cell {
            ...
            int dist;  // the distance of the cell from the initial position of the robot
                       // to the label that updates the Dijkstra's algorithm
            Cell prev; // Each state corresponds to a cell
                       // and each state has a predecessor which
                       // is stored in this variable
            
            boolean foundMe; // flag to see if this cell has been found by the search
        ...
        private class MouseHandler implements MouseListener, MouseMotionListener {
            ...
            public void mousePressed(MouseEvent evt) {
                ...
                if (row >= 0 && row < rows && col >= 0 && col < columns) {
                    if (realTime ? true : !foundAllTargets() && !searching){
            ...
            /*
             * Function that takes a row and column integer and searches
             * through the targets list for a target matching those
             * coordinates, if one is found it returns that cells index,
             * otherwise it returns -1.
             */
            public int findTarget(int r, int c) {
            	for (int i = 0; i < targets.size(); i++) {
            		if (targets.get(i).row == r && targets.get(i).col == c)
            			return i;
            	}
				return -1;
            }
            ...
            public void mouseDragged(MouseEvent evt) {
                int rows = (evt.getY() - 10) / squareSize;
                int cols = (evt.getX() - 10) / squareSize;
                if (rows >= 0 && rows < rows && cols >= 0 && cols < columns){
                    if (realTime ? true : !foundAllTargets() && !searching){
                        if (realTime) {
                            searching = true;
                            fillGrid();
                        }
                        if ((rows*columns+cols != cur_row*columns+cur_col) && (cur_val == ROBOT || cur_val == TARGET)){
                        	System.out.println("Target or Robot");
                            int new_val = grid[rows][cols];
                            if (new_val == EMPTY){
                                grid[rows][cols] = cur_val;
                                if (cur_val == ROBOT) {
                                    robotStart.row = rows;
                                    robotStart.col = cols;
                                } else {
                                	int index = findTarget(rows, cols);
                                	if (index != -1) {
                                		Cell tempCell = targets.get(index);
                                		tempCell.row = rows;
                                		tempCell.col = cols;
                                		targets.set(index, tempCell);
                                	} // if a target at the coords is found the change its row and col number
                                	else {
                                		System.out.println("not a target, line 235 error");
                                	} // else warn of error
                                }
                                ...
                                if (cur_val == ROBOT) {
                                	...
                                	if (index != -1) {
                                		Cell tempCell = targets.get(index);
                                		tempCell.row = cur_row;
                                		tempCell.col = cur_col;
                                		targets.set(index, tempCell);
                                	} // if a target at the coords is found the change its row and col number
                                	else {
                                		System.out.println("not a target, line 253 error");
                                	} // else warn of error
                                }
                                cur_val = grid[rows][cols];
                            }
                        } else if (grid[rows][cols] != ROBOT && grid[rows][cols] != TARGET){
                            grid[rows][cols] = OBST;
                        }
                        if (realTime) {
                            if (dijkstra.isSelected()) {
        ...
        private class ActionHandler implements ActionListener {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if (cmd.equals("Clear")) {
                    ...
                    checkTermination();
                } else if (cmd.equals("Step-by-Step") && !foundAllTargets() && !endOfSearch) {
                    realTime = false;
        ...
        JSpinner rowsSpinner, columnsSpinner, targetsSpinner; // Spinners for entering # of rows and columns and targets
        
        int rows    = 41,           // the number of rows of the grid
            columns = 41,           // the number of columns of the grid
            numTargets = 1,			// the default number of targets on the grid
        ...
        ArrayList<Cell> targets	  = new ArrayList<Cell>();  // the position of the targets
        //Cell targetPos;
        //Cell targetPos2;
        ...
        // Search with bidirectional
        JCheckBox bidirectional;
        // Add a subgoal to the maze
        //JCheckBox subGoal;
        ...
        public MazePanel(int width, int height) {
            ...
            // label and box for number of rows
            ...
            // label and box for number of columns
            ...
            // label and box for number of targets
            JLabel targetsLbl = new JLabel("# of targets (1-10):", JLabel.RIGHT);
            targetsLbl.setFont(new Font("Helvetica",Font.PLAIN,13));
            SpinnerModel targetModel = new SpinnerNumberModel(1, //initial value
                                       1,  //min
                                       10, //max
                                       1); //step
            targetsSpinner = new JSpinner(targetModel);
            targetsSpinner.addChangeListener(new ChangeListener() {
                @Override
                public void stateChanged(ChangeEvent e) {
                	int targetsVal = (int)targetsSpinner.getValue();
                	//System.out.println("targets value changed to: " + targetsVal);
                	numTargets = (int)(targetsSpinner.getValue());
                	if (targetsVal > targets.size()) {
                		//System.out.println("Adding a sub goal");
		                addSubGoal();
                	} else if (targetsVal < targets.size()){
                		//System.out.println("Removing a sub goal");
	                	removeSubGoal();
                	}
                }
            });
            ...
            saveButton.setToolTipText
                    ("Saves the maze to an output file for later use");
            ...
            loadButton.setToolTipText
                    ("Loads a maze from a txt save file");
            ...
            bidirectional = new
                    JCheckBox("Bidirectional Search");
            bidirectional.setToolTipText("Searches bidirecitonally with the selected search");
            ...
            targetsLbl.setBounds(520, 65, 130, 25);
            targetsSpinner.setBounds(655, 65, 35, 25);
            resetButton.setBounds(520, 95, 170, 25);
            mazeButton.setBounds(520, 125, 170, 25);
            clearButton.setBounds(520, 155, 170, 25);
            realTimeButton.setBounds(520, 185, 170, 25);
            stepButton.setBounds(520, 215, 170, 25);
            animationButton.setBounds(520, 245, 170, 25);
            velocity.setBounds(520, 275, 170, 10);
            slider.setBounds(520, 285, 170, 25);
            dfs.setBounds(530, 330, 70, 25);
            bfs.setBounds(600, 330, 70, 25);
            aStar.setBounds(530, 355, 70, 25);
            greedy.setBounds(600, 355, 85, 25);
            dijkstra.setBounds(530, 380, 85, 25);
            algoPanel.setLocation(520,310);
            algoPanel.setSize(170, 130);
            diagonal.setBounds(520, 415, 170, 25);
            drawArrows.setBounds(520, 440, 170, 25);
            bidirectional.setBounds(520, 465, 170, 25);
            robot.setBounds(520, 490, 80, 25);
            target.setBounds(605, 490, 80, 25);
            frontier.setBounds(520, 510, 80, 25);
            closed.setBounds(605, 510, 80, 25);
            aboutButton.setBounds(520, 540, 170, 25);
            saveButton.setBounds(520, 570, 170, 25);
            loadButton.setBounds(520, 600, 170, 25);
    ...
    static protected JSpinner addLabeledSpinner(Container c,
        ...
        private void mazeButtonActionPerformed(java.awt.event.ActionEvent evt) {
        	//System.out.println("Num of targets when maze button clicked is = " + targets.size());
        	for(int i = targets.size(); i > 1; i--) {
        		removeSubGoal();
        	} // remove all but 1 subgoals from the maze so we can generate it correctly
            realTime = false;
            realTimeButton.setEnabled(true);
            realTimeButton.setForeground(Color.black);
            stepButton.setEnabled(true);
        ...
        private void loadButtonActionPerformed(java.awt.event.ActionEvent evt) {
            ...
            int co = 0;	// columns in load file
            int ro = 0; // rows in load file
            ...
            // find out the row and column length and change it accordingly
        	try {
            	reader = new BufferedReader(new FileReader(loadFilePath));
                String line = reader.readLine();
                while (line != null) {
	                String noSlashes = line.replace("\\", "");
	            	arr = noSlashes.toCharArray();
        ...
        public void addSubGoal() {
            ...
            rand_Row = rand.nextInt(rows - 2); 
            rand_Col = rand.nextInt(columns - 2);
            boolean flag = true;
            
            // Generate a random subgoal
            while(flag) {
            	if(grid[rand_Row][rand_Col] == EMPTY) {
                	targets.add(new Cell(rand_Row,rand_Col));
                	grid[rand_Row][rand_Col] = TARGET; 
                	//System.out.println("Adding target at: " + rand_Row + "," + rand_Col);
                	flag = false;
               }
               else {
            	   rand_Row = rand.nextInt(rows - 2); 
            ...
            //System.out.println("Adding coords: " + targetPos2.row + "," + targetPos2.col);
            repaint();
        }
        
        public void removeSubGoal() {
        	//System.out.println("Removing coords: " + targetPos2.row + "," + targetPos2.col);
        	// if we still have targets in the list then remove the last one
        	if (targets.size() > 1) {
        		//System.out.println("Removing subgoal");
        		grid[targets.get(targets.size() - 1).row][targets.get(targets.size() - 1).col] = EMPTY; 
        ...
        public void emptyGrid() {
        	//System.out.println("Emptying grid");
        	for(int r = 0; r < rows; r++) {
        		for(int c = 0; c < columns; c++) {
        			grid[r][c] = EMPTY;
        		}
        ...
        /**
         * Function that prints the grid to the console, used for debugging
         */
        public void printGrid() {
        	...
        	System.out.println("Num of goals found = " + count);
        }
        ...
        private void initializeGrid(Boolean makeMaze) {    
            ...
            grid = new int[rows][columns];
            //System.out.println("Adding start robot");
            ...
            drawArrows.setEnabled(true);
            bidirectional.setSelected(false);
            bidirectional.setEnabled(true);
            slider.setValue(500);
            if (makeMaze) {
                ...
                fillGrid();
                //System.out.println("else fill grid");
            }
            for (int i = targets.size(); i < numTargets; i++) {
            	addSubGoal();
            }
            //printGrid();
        ...
        /*
         * Loops through all the targets in the array list
         * checking if all of them have been discovered in
         * which case it returns true, if not it returns
         * false.
         */
        private boolean foundAllTargets() {
        	int foundCount = 0;
        	for(int i = 0; i < targets.size(); i++) {
        		if(targets.get(i).foundMe == true) {
        			foundCount++;
        		}
        	}
        	if (foundCount == targets.size()) {
        		//System.out.println("Found all targets");
        		return true;
        	} else {
        		//System.out.println("Not all targets found");
        		return false;
        	}
        }
        ...
        private void expandNode(){
            ...
            if (dijkstra.isSelected()){
                ...
                int cellIndex = findTarget(u.row, u.col);
                if (cellIndex != -1){
                    Cell temp = targets.get(cellIndex);
                    //System.out.println("Taget at coords (" + temp.row + "," + temp.col + ") has been found");
                    temp.foundMe = true;
                    targets.set(cellIndex, temp);
                }
                if(foundAllTargets()) {
                	return;
                } // if we have found all the targets then return
                ...
                // If target has been found ...
                int cellIndex = findTarget(current.row, current.col);
                if (cellIndex != -1){
                    Cell temp = targets.get(cellIndex);
                    Cell last = temp;
                ...
                succesors.stream().forEach((cell) -> {
                    ...
                    if (dfs.isSelected()) {
                        ...
                        int dxh = targets.get(0).col-cell.col;
                        int dyh = targets.get(0).row-cell.row;
        ...
        private void plotRoute(){
            ...
            for(int i = 0; i < targets.size(); i++) {
            	index.add(isInList(closedSet,targets.get(i)));
            } // gets all the indexes of where the target is in the closed set
            for(int i = 0; i < index.size(); i++) {
            	cur.add(closedSet.get(index.get(i)));
            } // gets all the targets from the closed set
            for(int i = 0; i < targets.size(); i++) {
	            do {
	                steps++;
	                if (diagonal.isSelected()) {
	                    int dx = cur.get(i).col-cur.get(i).prev.col;
	                    int dy = cur.get(i).row-cur.get(i).prev.row;
	                    distance += Math.sqrt(dx*dx + dy*dy);
	                } else { 
	                    distance++;
	                }
	                cur.set(i, cur.get(i).prev);
	                grid[cur.get(i).row][cur.get(i).col] = ROUTE;
	            } while (!(cur.get(i).row == robotStart.row && cur.get(i).col == robotStart.col));
            }
            grid[robotStart.row][robotStart.col] = ROBOT;
            for(int i = 0; i < targets.size(); i++) {
            	grid[targets.get(i).row][targets.get(i).col] = TARGET;
            }
            String msg;
        ...
        /**
         * Gives initial values ​​for the cells in the grid.
...
         */
        private void fillGrid() {
        	Random rand = new Random();
            int rand_Row = rand.nextInt(rows - 2); 
            int rand_Col = rand.nextInt(columns - 2);
            boolean flag = true;
            int targetIndex = 0;
            ...
            if (searching || endOfSearch){ 
                for (int r = 0; r < rows; r++) {
                    for (int c = 0; c < columns; c++) {
                        ...
                        while (grid[r][c] == TARGET && targetIndex < targets.size()){
                            targets.set(targetIndex, new Cell(r,c));
                            targetIndex++;
                        } // if were at a target and we have not set all our targets yet then keep going
                    }
                }
                searching = false;
            } else {
                ...
                robotStart = new Cell(1,1);
                if (targets.isEmpty()) {
                	//System.out.println("targets is empty, adding a target");
                	targets.add(new Cell(rows - 2, columns - 2));
                } // if we have no targets then add one at the end of the maze
                else {
                	targets.set(0, new Cell(rows - 2, columns - 2));
                } // the first cell in targets should always be at the end of the maze
//                if (targets.size() > 1) {
...
//                }
                ...
                for (int i = targets.size(); i < numTargets; i++) {
                	...
                	if (grid[rand_Row][rand_Col] == EMPTY) {
                		targets.add(new Cell(rand_Row,rand_Col));
                		//System.out.println("Num of targets: " + targets.size());
                	}
                	else {
                		rand_Row = rand.nextInt(rows - 2); 
                    	rand_Col = rand.nextInt(columns - 2);
                	}
                } // set the rest of the targets to a random open positions
            ...
            for (int i = 0; i < targets.size(); i++) {
            	...
            	temp.foundMe = false;
            	targets.set(i, temp);
            } // set all target found values to false
            searching = false;
            endOfSearch = false;
            ...
            for (int i = 0; i < targets.size(); i++) {
            	grid[targets.get(i).row][targets.get(i).col] = TARGET;
            	//System.out.println("Target set at (" + targets.get(i).row + "," + targets.get(i).col + ")");
            } // set all targets to the correct target value of 3
            //printGrid();
            grid[robotStart.row][robotStart.col] = ROBOT;
